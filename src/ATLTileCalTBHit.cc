...
//
ATLTileCalTBHit::ATLTileCalTBHit() : G4VHit(), fEdep(0.), fSdepUp(), fSdepDown()
{
  fSdepUp.fill(0.);
  fSdepDown.fill(0.);
}
...
ATLTileCalTBHit::ATLTileCalTBHit(const ATLTileCalTBHit& right) : G4VHit()
{
  fEdep = right.fEdep;
...
std::size_t ATLTileCalTBHit::GetBinFromTime(G4double time)
{
  ...
  if (time < frame_time_window) {
    ...
    G4Exception("ATLTileCalTBHit::GetBinFromTime()", "MyCode0008", FatalException, msg);
