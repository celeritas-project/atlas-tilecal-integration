    ...
    fAux{0., 0.}
{
  ...
  fXEdepVector = std::vector<G4double>(100, 0.); // see XMin/XMax
}
...
void ATLTileCalTBEventAction::BeginOfEventAction([[maybe_unused]] const G4Event* event)
{
  ...
  pulse_event_path = std::filesystem::path(
    "ATLTileCalTBpulse_Run" + std::to_string(runNumber) + "/Ev" + std::to_string(eventNumber));
...
//
ATLTileCalTBHitsCollection* ATLTileCalTBEventAction::GetHitsCollection(
  G4int hcID, const G4Event* event) const
{
  auto hitsCollection =
    static_cast<ATLTileCalTBHitsCollection*>(event->GetHCofThisEvent()->GetHC(hcID));

  if (! hitsCollection) {
    G4ExceptionDescription msg;
    msg << "Cannot access hitsCollection ID " << hcID;
    G4Exception("ATLTileCalTBEventAction::GetHitsCollection()", "MyCode0003", FatalException, msg);
  }
...
void ATLTileCalTBEventAction::EndOfEventAction(const G4Event* event)
{
  ...
  // From
  // https://gitlab.cern.ch/allpix-squared/allpix-squared/-/blob/86fe21ad37d353e36a509a0827562ab7fadd5104/src/modules/CSADigitizer/CSADigitizerModule.cpp#L271-L283
...
// Create output pulses if requested
  ...
  analysisManager->FillNtupleIColumn(
    6, fPrimaryGenAction->GetParticlenGun()->GetParticleDefinition()->GetPDGEncoding());
